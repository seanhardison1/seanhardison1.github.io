---
title: Lower Trophic Levels
author: Ecosystems Dynamics and Assessment
date: '2018-11-26'
slug: lower-trophic-levels
categories: []
tags: []
output:
  blogdown::html_page:
...
#Data directories
raw.dir <- here("static","inst","extdata") #raw data directory
clean.dir <- here("static","data") #output directory for cleaned data
gis.dir <- here("static","inst","extdata","gis")
sample.dir <- here("static","inst","extdata","sample") 
...
zoo_plt <- ggplot(data = zoo, aes(x = Time, y = Value)) +
    ...
    else geom_point(aes(x = Time, y = Value))}+
    facet_wrap(Var ~ ., nrow = 3, ncol = 2, labeller = label) +
    theme_bw() +
    theme(strip.background = element_blank()) +
    annotate("text", label = c("","","",
...
N <- NE_LME_nutrients %>% 
  ...
  {if(gg_int) geom_point_interactive(aes(tooltip = round(Value,3), data_id = Time,x = Time, y = Value, color = EPU))
    else geom_point(aes(x = Time, y = Value, color = EPU))}+
